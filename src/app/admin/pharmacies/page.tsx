...
import { Building2, Search, AlertCircle, RefreshCw, CheckCircle, XCircle, Eye, ChevronLeft, ChevronRight, MapPin } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { useGetAdminPharmaciesQuery, useUpdatePharmacyVerificationMutation } from "@/store/api/adminApi";
...
export default function AdminPharmaciesPage() {
  ...
  const { data, isLoading, error, refetch } = useGetAdminPharmaciesQuery({
  ...
  const handleVerify = async (pharmacyId: string, verified: boolean) => {
    try {
      await updateVerification({ pharmacyId, verified }).unwrap();
    } catch {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Pharmacies</h1>
        <p className="text-zinc-600 dark:text-zinc-400">Manage pharmacy profiles and verification.</p>
      </div>
        ...
        <Card>
          <CardContent className="py-8">
            <div className="flex flex-col items-center justify-center">
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-3">Failed to load pharmacies.</p>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
            {data.data.length === 0 ? (
              ...
              data.data.map((pharmacy) => (
                <div key={pharmacy.id} className="flex flex-col md:flex-row md:items-center justify-between px-6 py-4 border-b border-zinc-100 dark:border-zinc-800 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                      <span className="flex items-center gap-1">
                        ...
                        {pharmacy.address.length > 40 ? pharmacy.address.slice(0, 40) + "..." : pharmacy.address}
                      </span>
                      <span>·</span>
                      <span>{pharmacy.counts.medicines} medicines</span>
                      <span>·</span>
                      <span>{pharmacy.counts.staff} staff</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Link
          ...
          {data.meta && data.meta.totalPages > 1 && (
            <div className="flex items-center justify-between">
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Showing {(page - 1) * limit + 1}–{Math.min(page * limit, data.meta.total)} of {data.meta.total} pharmacies
              </p>
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
